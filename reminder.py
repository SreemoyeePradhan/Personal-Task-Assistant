...
import schedule
from assistant import load_tasks, save_tasks, speak, notify
...
def check_tasks():
    ...
    for task in tasks:
        ...
        # Trigger when due time equals current minute and not yet done
        if not done and due == now_str:
            msg = f"Reminder: {desc}"
            print(f"🔔 {msg}")
            notify("Task Reminder", msg)
            speak(msg)
            # Mark as done automatically after reminding
            task["done"] = True
            updated = True
